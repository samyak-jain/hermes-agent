...
# Composio is service-gated on composio.enabled + credentials. Keep it on
# user-driven surfaces, but out of cron and webhook toolsets where external
# input may initiate a turn without a person present.
_HERMES_USER_TOOLS = _HERMES_CORE_TOOLS + ["composio"]
...
TOOLSETS = {
    ...
    "hermes-api-server": {
        ...
        "tools": [
            ...
            # Cronjob management
            "cronjob",
            # Service-gated external actions (operator opt-in + app allowlist)
            "composio",
            # Home Assistant smart home control (gated on HASS_TOKEN via check_fn)
            "ha_list_entities", "ha_get_state", "ha_list_services", "ha_call_service",
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    ...
    Platform bundles are defined as ``_HERMES_USER_TOOLS + [platform extras]``.
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        ...
        # Gives them _HERMES_USER_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("hermes-"):
            ...
            try:
                ...
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_HERMES_USER_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
