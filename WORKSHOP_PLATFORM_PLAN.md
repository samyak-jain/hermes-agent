...
Re-review remediation at `39a7e85`: current-turn remote-tool authority is now enforced at the DO publication boundary as well as the Hermes dispatcher, so a cache-reused stale shim cannot expose a tool call during a zero-tool delta/wake turn. Backlog exhaustion terminalizes synchronously at the durable drop site even when the production streaming callback swallows exceptions. Workshop slash-command exclusion is structural in `MessageEvent.is_command()` and no longer depends on a separately configurable flag. `PROGRESS.md` records real HTTP/adapter/runtime-path coverage and revert-the-fix mutation evidence for R1-R3.

The re-review repository run also caught and corrected an R3 base-contract regression before push: `MessageEvent.source` is optional for generic/source-less events. The final structural guard excludes commands only when an actual Workshop source is present and preserves legacy behavior for source-less and all non-Workshop events. The full 194-test platform-base file and all changed adjacent surfaces pass together.
...
- Autonomous producers have no fresh caller-granted tool catalog, so their turns deliberately receive zero remote workshop schemas and no callback authority. They reuse only the chat's established digest to avoid prompt-cache churn; a digest is cache identity, not a capability grant.
...
- **Permanent invisible-Unicode delta rejection:** if the safety scanner rejects content such as an unpaired U+200B, that `delta_id` must be considered permanently rejected. The DO must sanitize/reframe the workspace content and submit a newly allocated identity; retrying the same rejected delta is not a recovery mechanism.
- **Kumo fork interrupt contract:** `AIAgent.interrupt()` now signals a live Codex app-server session in addition to the established Python interrupt flags. Discord routing/authorization semantics and cron's inline request-abort behavior are unchanged, but interruption becomes effective sooner on the production runtime. Keep this explicit in `hermes-fork.md` and exercise one live Discord interrupt plus one cron turn before deployment.
