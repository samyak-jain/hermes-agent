"""Profile-scoped, audited persistence for the operator's ``GOALS.md``.
...
class GoalsError(RuntimeError):
    """A goals operation was rejected without changing the file."""


class GoalsConflict(GoalsError):
...
def _policy_block(config: Optional[dict] = None) -> Optional[dict]:
    ...
    block = config.get("goals_edit") if isinstance(config, dict) else None
...
def configured_max_bytes(config: Optional[dict] = None) -> int:
    """Return the fixed, bounded GOALS byte ceiling."""
...
def _secure_dir(path: Path, *, create: bool = False, mode: int = 0o700) -> None:
    ...
    try:
        ...
        raise GoalsError(f"Required profile directory does not exist: {path}") from exc
...
def _open_at(
    ...
    try:
        ...
        raise GoalsError(f"Could not securely open {home / name}: {exc}") from exc


def _assert_regular(info: os.stat_result, path: Path) -> None:
    if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
        raise GoalsError(f"Refusing symlink or non-regular GOALS file: {path}")
...
def _read_target(
    ...
    path = home / "GOALS.md"
    before = _stat_at(home, home_fd, "GOALS.md")
    if before is None:
        return b"", False, None
    _assert_regular(before, path)
    fd = _open_at(home, home_fd, "GOALS.md", os.O_RDONLY)
    try:
        opened = os.fstat(fd)
        _assert_regular(opened, path)
        if (opened.st_dev, opened.st_ino) != (before.st_dev, before.st_ino):
            raise GoalsConflict(
                "GOALS.md changed while it was being opened; retry the read."
            )
        chunks: list[bytes] = []
        while True:
...
def _atomic_write(
    ...
    *,
    ...
    temp_name = f".GOALS.md.{secrets.token_hex(8)}.tmp"
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:  # pragma: no cover - defensive OS invariant
                raise GoalsError("Short write while persisting GOALS.md.")
            view = view[written:]
        if previous is not None:
            ...
            # GOALS.md is operator-owned profile state, not a credential file.
        ...
        if _version(current_data, exists=current_exists) != expected_version:
            raise GoalsConflict(
                "GOALS.md changed during the atomic replacement. Nothing was written; "
                "read it again and retry."
            )
        if home_fd is not None:
            os.replace(temp_name, "GOALS.md", src_dir_fd=home_fd, dst_dir_fd=home_fd)
            os.fsync(home_fd)
        else:  # pragma: no cover - Windows
            os.replace(home / temp_name, home / "GOALS.md")
    except GoalsError:
        raise
    except OSError as exc:
        raise GoalsError(f"Atomic GOALS.md replacement failed: {exc}") from exc
    finally:
        ...
        try:
            ...
            logger.warning("Could not clean up temporary GOALS file %s", temp_name)
...
def _atomic_remove(home: Path, home_fd: int | None, *, expected_version: str) -> None:
    ...
    try:
        if home_fd is not None:
            os.unlink("GOALS.md", dir_fd=home_fd)
            os.fsync(home_fd)
        else:  # pragma: no cover - Windows
            (home / "GOALS.md").unlink()
    except OSError as exc:
        raise GoalsError(
            f"Could not atomically restore an absent GOALS.md: {exc}"
        ) from exc


@contextmanager
def _locked_home(home: Path) -> Iterator[int | None]:
    """Serialize every known GOALS writer for one profile."""
    _secure_dir(home)
    with _PROCESS_LOCK:
        home_fd = _open_directory(home)
        lock_fd: int | None = None
        try:
            lock_info = _stat_at(home, home_fd, ".GOALS.md.lock")
            if lock_info is not None and not stat.S_ISREG(lock_info.st_mode):
                raise GoalsError("Refusing symlink or non-regular GOALS lock file.")
            lock_fd = _open_at(
                home,
                home_fd,
                ".GOALS.md.lock",
            ...
            if not stat.S_ISREG(os.fstat(lock_fd).st_mode):
                raise GoalsError("Refusing non-regular GOALS lock file.")
            try:
...
def _state_paths(home: Path, *, create: bool) -> tuple[Path, Path]:
    state = home / "state"
    goals_state = state / "goals"
    revisions = goals_state / "revisions"
    if create:
        _secure_dir(state, create=True)
        _secure_dir(goals_state, create=True)
        _secure_dir(revisions, create=True)
    else:
        for path in (state, goals_state, revisions):
            try:
                path.lstat()
            except FileNotFoundError:
                break
            _secure_dir(path)
    return goals_state, revisions
...
def _safe_open_file(path: Path, flags: int, mode: int = 0o600) -> int:
    ...
    try:
        ...
        raise GoalsError(
            f"Could not securely open GOALS state file {path}: {exc}"
        ) from exc
    if not stat.S_ISREG(os.fstat(fd).st_mode):
        os.close(fd)
        raise GoalsError(f"Refusing non-regular GOALS state file: {path}")
    return fd


def _write_backup(home: Path, revision: str, before: bytes) -> Path:
    _state, revisions = _state_paths(home, create=True)
    path = revisions / f"{revision}.goals"
    ...
    try:
        ...
        while view:
            ...
            if written <= 0:  # pragma: no cover
                raise GoalsError("Short write while snapshotting GOALS.md.")
            view = view[written:]
        os.fsync(fd)
    finally:
        os.close(fd)
...
def _append_audit(home: Path, record: dict[str, Any]) -> None:
    goals_state, _revisions = _state_paths(home, create=True)
    path = goals_state / "audit.jsonl"
    encoded = (
    ...
    try:
        ...
        while view:
            written = os.write(fd, view)
            if written <= 0:  # pragma: no cover - defensive OS invariant
                raise GoalsError("Short write while persisting the GOALS audit log.")
            view = view[written:]
        os.fsync(fd)
    finally:
        os.close(fd)
...
def _read_audit(home: Path) -> list[dict[str, Any]]:
    goals_state, _revisions = _state_paths(home, create=False)
    path = goals_state / "audit.jsonl"
    try:
        info = path.lstat()
    except FileNotFoundError:
    ...
    if not stat.S_ISREG(info.st_mode) or stat.S_ISLNK(info.st_mode):
        raise GoalsError("Refusing symlink or non-regular GOALS audit log.")
    fd = _safe_open_file(path, os.O_RDONLY)
    try:
        with os.fdopen(fd, "r", encoding="utf-8", errors="strict") as handle:
            lines = handle.read().splitlines()
    except UnicodeDecodeError as exc:
        raise GoalsError("GOALS audit history is not valid UTF-8.") from exc
...
def _read_backup(home: Path, revision: str) -> bytes:
    ...
    try:
        ...
        raise GoalsError(
            f"GOALS revision {revision!r} is unavailable or outside the retained five."
        ) from exc
    if not stat.S_ISREG(info.st_mode) or stat.S_ISLNK(info.st_mode):
        raise GoalsError("Refusing symlink or non-regular GOALS revision file.")
    fd = _safe_open_file(path, os.O_RDONLY)
...
def _remove_backup(home: Path, revision: str) -> None:
    ...
    try:
        info = path.lstat()
        if not stat.S_ISREG(info.st_mode) or stat.S_ISLNK(info.st_mode):
            raise GoalsError("Refusing non-regular orphaned GOALS revision file.")
        path.unlink()
    except FileNotFoundError:
        pass
...
def _prune_revisions(home: Path, records: list[dict[str, Any]]) -> Optional[str]:
    ...
    for path in revisions.iterdir():
        match = re.fullmatch(r"(\d{8}T\d{6}Z-[0-9a-f]{8})\.goals", path.name)
        if not match or match.group(1) in keep:
            continue
        try:
            info = path.lstat()
            if not stat.S_ISREG(info.st_mode) or stat.S_ISLNK(info.st_mode):
                raise GoalsError("revision path is not a regular file")
            path.unlink()
        except Exception as exc:  # pruning is post-commit and must not undo GOALS
            failures.append(f"{path.name}: {exc}")
    if not failures:
        return None
    warning = (
        "GOALS.md was committed, but one or more old revisions could not be pruned: "
        + "; ".join(failures)
    )
...
def _retained_revision_count(home: Path) -> int:
    ...
    count = 0
    for path in entries:
        if not re.fullmatch(r"\d{8}T\d{6}Z-[0-9a-f]{8}\.goals", path.name):
            continue
        info = path.lstat()
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISREG(info.st_mode):
            raise GoalsError("Refusing symlink or non-regular GOALS revision file.")
...
def _validate_reason(reason: str) -> str:
    ...
    if not reason:
        raise GoalsError("A concise operator-request reason is required.")
    if len(reason) > _MAX_REASON_CHARS:
        raise GoalsError(
            f"The GOALS change reason must be at most {_MAX_REASON_CHARS} characters."
        )
    from tools.threat_patterns import scan_for_threats

    if "hardcoded_secret" in scan_for_threats(reason, scope="strict"):
        raise GoalsError(
            "The GOALS change reason appears to contain credential material."
        )
    return reason
...
def _validate_content(content: str, *, max_bytes: int) -> bytes:
    if not isinstance(content, str):
        raise GoalsError("GOALS content must be a Unicode string.")
    if not content.strip():
        raise GoalsError("GOALS content cannot be empty.")
    if "\x00" in content:
        raise GoalsError("GOALS content cannot contain NUL characters.")
    try:
        encoded = content.encode("utf-8", errors="strict")
    except UnicodeEncodeError as exc:
        raise GoalsError("GOALS content is not valid UTF-8 Unicode.") from exc
    if len(encoded) > max_bytes:
        raise GoalsError(
            f"GOALS content is {len(encoded):,} bytes; the configured limit is "
            f"{max_bytes:,} bytes."
        )
    from tools.threat_patterns import scan_for_threats
    ...
    if findings:
        raise GoalsError(
            "GOALS content matched context-security checks: " + ", ".join(findings)
        )
    return encoded
...
def _assert_writable(home: Path, config: Optional[dict] = None) -> tuple[str, str]:
    ...
    if not editable:
        raise GoalsError(reason or "GOALS.md is not profile-editable.")
    ...
    *,
        ...
        if latest_version == after_version:
            try:
                ...
                if operation == "rollback":
                    raise GoalsError(
                        "GOALS.md rollback reported a mutation failure after changing "
                        "the file, and exact restoration also failed. "
                        f"Rollback: {mutation_exc}; restore: {restore_exc}"
                    ) from restore_exc
                raise GoalsError(
                    "GOALS.md replacement reported a durability failure after "
                    "changing the file, and exact restoration also failed. "
                    f"Replacement: {mutation_exc}; restore: {restore_exc}"
                ) from restore_exc
            if operation == "rollback":
                raise GoalsError(
                    "GOALS.md rollback reported a mutation failure, so the "
                    "pre-rollback state was restored."
                ) from mutation_exc
            raise GoalsError(
                "GOALS.md replacement reported a durability failure, so the "
                "original content was restored."
            ) from mutation_exc
        if operation == "rollback":
            raise GoalsError(
                "GOALS.md changed unexpectedly during a failed rollback. The "
                "pre-rollback snapshot was retained for manual reconciliation."
            ) from mutation_exc
        raise GoalsError(
            "GOALS.md changed unexpectedly during a failed replacement. The "
    ...
    try:
        ...
        try:
            ...
            if operation == "rollback":
                raise GoalsError(
                    "GOALS rollback audit failed, and restoration of the pre-rollback "
                    f"state also failed. Audit: {audit_exc}; restore: {restore_exc}"
                ) from restore_exc
            raise GoalsError(
                "GOALS.md was written but audit persistence failed, and exact "
                f"restoration also failed. Reconcile it manually. Audit: {audit_exc}; "
                f"restore: {restore_exc}"
            ) from restore_exc
        if operation == "rollback":
            raise GoalsError(
                "GOALS rollback audit persistence failed, so the rollback was undone."
            ) from audit_exc
        raise GoalsError(
            "GOALS audit persistence failed, so the update was automatically restored."
        ) from audit_exc
    ...
    *, home: Optional[Path] = None, config: Optional[dict] = None
    ...
    with _locked_home(home) as home_fd:
        ...
        if len(data) > max_bytes:
            raise GoalsError(
                f"GOALS.md is {len(data):,} bytes; the configured limit is "
                f"{max_bytes:,} bytes."
            )
        content = _decode_existing(data, home / "GOALS.md") if exists else ""
    ownership, editable, read_only_reason = _ownership(home, config)
    return {
        "success": True,
        "profile": _profile_name(home),
        # Supply the canonical empty template without creating state on a read.
        # A first update that faithfully starts from this read therefore creates
        # a useful GOALS.md rather than an opaque blank file.
        "content": content if exists else DEFAULT_GOALS_MD,
        "exists": exists,
...
def update_goals(
    *,
    ...
    if not expected_version:
        raise GoalsError("expected_version from goals(action='read') is required.")

    with _locked_home(home) as home_fd:
        profile, ownership = _assert_writable(home, config)
        before, before_exists, before_info = _read_target(home, home_fd)
        if before_exists:
            _decode_existing(before, home / "GOALS.md")
        before_version = _version(before, exists=before_exists)
        if expected_version != before_version:
            raise GoalsConflict(
                "GOALS.md changed after it was read. Nothing was written; read it again and retry."
            )
        if before_exists and before == encoded:
            raise GoalsError("GOALS.md already has the requested content.")
    ...
    return {
        ...
        "message": "GOALS.md updated atomically.",
    }
...
def history_goals(
    *,
    ...
    try:
        ...
        raise GoalsError("History limit must be an integer from 1 to 100.") from exc
    if not 1 <= limit <= 100:
        raise GoalsError("History limit must be an integer from 1 to 100.")
...
def rollback_goals(
    *,
    ...
    if not _REVISION_RE.fullmatch(revision):
        raise GoalsError("A valid GOALS revision ID is required.")
    expected_version = str(expected_version or "").strip()
    if not expected_version:
        raise GoalsError("expected_version from goals(action='read') is required.")
    ...
    with _locked_home(home) as home_fd:
        ...
        if current_exists:
            _decode_existing(current, home / "GOALS.md")
        ...
        if target is None:
            raise GoalsError(f"Unknown GOALS revision {revision!r}.")
        restore_exists = bool(target.get("before_exists", True))
        restore = _read_backup(home, revision)
        if restore_exists:
            _decode_existing(restore, home / "GOALS.md revision")
            _validate_content(
                restore.decode("utf-8"),
                max_bytes=configured_max_bytes(config),
            )
        elif restore:
            raise GoalsError("Absent-file GOALS revision contains unexpected bytes.")
    ...
    return {
        ...
        "apply": "immediate",
        "prune_warning": prune_warning,
        "message": "GOALS.md rollback applied atomically.",
    }
