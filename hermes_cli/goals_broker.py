...
class GoalsError(RuntimeError):
    """An operator-goals operation was rejected without changing the file."""
...
def configured_max_bytes(config: Optional[dict] = None) -> int:
    """Return the fixed operator-goals byte ceiling."""
...
def _validate_content(content: str, *, max_bytes: int) -> bytes:
    ...
    if _looks_like_secret(content):
        raise GoalsError("GOALS content appears to contain credential material.")
    return encoded
...
def _ownership(home: Path, config: Optional[dict] = None) -> tuple[str, bool, str]:
    ...
    if manifest_path.exists():
        ...
        if "GOALS.md" in owned:
            return (
                "distribution",
                False,
                "GOALS.md is owned by this profile distribution.",
            )
    return "profile", True, ""
...
def update_goals(
    *,
    ...
    return {
        ...
        "message": "GOALS.md updated atomically and is available immediately.",
    }
...
def rollback_goals(
    *,
    ...
    return {
        ...
        "apply": "immediate",
        "prune_warning": prune_warning,
        "message": "GOALS.md rollback applied atomically and is available immediately.",
    }
