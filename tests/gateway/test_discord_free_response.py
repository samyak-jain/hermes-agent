...
async def test_ambient_history_is_full_room_window_including_bots(
    ...
    human = SimpleNamespace(id=42, display_name="Jezza", name="Jezza", bot=False)
    channel = FakeHistoryChannel(
        [
...
def test_ambient_room_invalid_hop_limit_falls_back_safely(adapter):
    ...
    message = make_message(
        channel=FakeTextChannel(channel_id=789), content="hello room"
    )
...
def test_ambient_room_accepts_only_bots_registered_by_this_gateway(adapter):
    ...
    accepted, _ = adapter._discord_message_admission(
        registered_message, claim=False
    )
    rejected, _ = adapter._discord_message_admission(
        unrelated_message, claim=False
    )
