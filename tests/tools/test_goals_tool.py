...
def test_read_uses_active_server_profile(tmp_path: Path, monkeypatch):
    ...
    (default / "GOALS.md").write_text("default identity", encoding="utf-8")
    (named / "GOALS.md").write_text("named identity", encoding="utf-8")
    monkeypatch.setattr(goals_tool_module, "tool_enabled", lambda: True)
    ...
    assert (default / "GOALS.md").read_text(encoding="utf-8") == "default identity"
...
def test_disabled_policy_fails_closed(monkeypatch):
    ...
    assert result == {
        "success": False,
        "error": "Operator-goals access is disabled by operator policy.",
    }
...
def test_tool_is_service_gated_and_not_in_core(monkeypatch):
    ...
    monkeypatch.setattr(goals_tool_module, "tool_enabled", lambda: True)
    invalidate_check_fn_cache()
    enabled = model_tools.get_tool_definitions(enabled_toolsets=["goals"])
