...
Four calling shapes:
  ...
  3. DISCOVERY — pass query → FTS5 + adaptive/full hydration
...
class TestBrowseShape:
    ...
    def test_browse_returns_ending_preview(self, db):
        ...
        result = json.loads(session_search(db=db))
        newest = next(
            r for r in result["results"] if r["session_id"] == "s_newest"
        )
    ...
    def test_previous_session_uses_conversation_affinity_not_keyword_guess(self, db):
        ...
        db.create_session(
            "current_dm", source="discord", session_key=dm_key, user_id="42"
        )
        db.append_message(
            "current_dm",
            role="user",
