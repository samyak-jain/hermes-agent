...
def test_creation_and_rollback_restore_absence(tmp_path: Path):
    ...
    changed = update_goals(
        home=home,
        config=POLICY,
        content=before["content"] + "## Ship the project\n\nFinish the first release.\n",
        expected_version="missing",
        reason="record the operator's stated goal",
    )
    assert (home / "GOALS.md").read_text(encoding="utf-8").startswith("# Goals\n\n")
    rolled_back = rollback_goals(
        home=home,
        config=POLICY,
...
def test_policy_defaults_fail_closed():
    ...
    assert tool_enabled({"goals_edit": {"enabled": False}}) is False
    assert tool_enabled({"goals_edit": {"enabled": True}}) is True
    assert approval_required({}) is True
    assert approval_required({"goals_edit": {"enabled": True}}) is True
    assert approval_required(
        {"goals_edit": {"enabled": True, "require_approval": False}}
    ) is False
...
def test_rejects_oversize_existing_file(goals_home: Path):
    (goals_home / "GOALS.md").write_bytes(b"x" * 65_537)
...
def test_rollback_mutation_failure_is_not_mislabeled_as_audit_failure(
    ...
    changed = _update(goals_home, "# Current before failed rollback\n")
    before = (goals_home / "GOALS.md").read_bytes()
    real_atomic_write = goals_module._atomic_write
    calls = 0
    ...
    monkeypatch.setattr(goals_module, "_atomic_write", mutate_then_fail)

    with pytest.raises(GoalsError, match="mutation failure") as error:
        rollback_goals(
            home=goals_home,
            config=POLICY,
            revision=changed["revision"],
    ...
    assert (goals_home / "GOALS.md").read_bytes() == before
    assert [
        item["operation"] for item in history_goals(home=goals_home)["changes"]
    ] == ["update"]
