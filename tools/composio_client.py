...
class ComposioSettings:
    ...
    def load(cls) -> "ComposioSettings":
        ...
        return cls(bool(block.get("enabled", False)), normalized, user_id, str(api_key).strip() if api_key else None)
...
class ComposioClient:
    ...
    def list_connections(self) -> list[dict[str, Any]]:
        ...
        return [_as_dict(item) for item in _items(response)]
    ...
    def delete_connection(self, connection_id: str) -> dict[str, Any]:
        ...
        # Fetch first and enforce the allowlist against the authoritative account.
        ...
        if not toolkit:
            raise ComposioError("Could not determine the connection's toolkit; refusing deletion.")
        self.require_app(toolkit)
        response = self.sdk.connected_accounts.delete(target, revoke_on_delete=True)
