...
def goals_tool(args: Mapping[str, Any], **context: Any) -> str:
    ...
    if not tool_enabled():
        return tool_error(
            "Operator-goals access is disabled by operator policy.",
            success=False,
        )
...
GOALS_SCHEMA = {
    "name": "goals",
    "description": (
        "Read or update only this running profile's operator-owned GOALS.md through "
        "a validated, audited broker. Goals belong to the operator: record or refine "
        "only goals the operator explicitly states in conversation, keep roughly "
        "eight terse goals at most, and never invent, reprioritize, or delete goals "
        "without an explicit operator statement. The profile is derived by Hermes; "
        "this tool never accepts a profile or path. Always read first and pass its "
        "expected_version when updating or rolling back. History is metadata-only."
    ),
    "parameters": {
        "type": "object",
...
registry.register(
    ...
    emoji="🎯",
    # Complete reads are required; worst-case JSON escaping is about 6x bytes.
